...
class ProductOfRedNodesVisitor extends TreeVis {
    ...
    public int getResult() {
        return (int) productRed;
    }

    private void compute(Tree tree){
        if(tree.getColor().equals(Color.RED)){
            productRed = (productRed *tree.getValue()) % MODULO;
        }
    }
...
class FancyVisitor extends TreeVis {
    ...
    public int getResult() {
        return Math.abs(sumGreen - sumEven);
    }
...
public class VisitorPattern {
    ...
    private static void DFSUtil(TreeNode parent, int v, boolean[] visited,
                                Map<Integer, ArrayList<Integer>> edges, Map<Integer, Object[]> nodeAtts) {
        visited[v] = true;
